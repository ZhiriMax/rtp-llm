...
void EmbeddingExecutor::fillKVCacheMetadata(GptModelInputs& model_input, size_t max_blocks_num) const {
    ...
    for (int32_t gid = 0; gid < kv_cache_group_num_; ++gid) {
        auto type = cache_config_.group_types.empty() ? CacheGroupType::FULL : cache_config_.group_types[gid];
...
absl::StatusOr<GptModelInputs> EmbeddingExecutor::gatherModelInput(const std::list<EmbeddingStreamPtr>& streams) const {
    ...
    if (max_blocks_num > 0) {
        ...
        model_input.kv_cache_kernel_block_id = torch::zeros({(int64_t)kv_cache_group_num_,
                                                             (int64_t)batch_size,
    ...
    for (auto& stream : streams) {
        ...
        if (embedding_input->prefix_lengths.defined()) {
            memcpy(prefix_lengths + (int)batch_idx,
                   embedding_input->prefix_lengths.data_ptr(),
                   stream->batchSize() * sizeof(int32_t));
        } else {
            std::fill(prefix_lengths + (int)batch_idx, prefix_lengths + (int)batch_idx + stream->batchSize(), 0);
        }
        int length_idx = 0;
...
absl::Status EmbeddingExecutor::processPrefixCacheStream(const EmbeddingStreamPtr& stream) {
    ...
    auto kv_resource = std::make_shared<BatchKVCacheResource>();
    kv_resource->resetBatchSize(batch_size);
    kv_resource->initGroups(kv_cache_group_num_,
    ...
    int max_row_len = 0;
    for (const auto& row : full_token_rows) {
        max_row_len = std::max(max_row_len, (int)row.size());
    }
    auto complete_token_ids =
        std::make_shared<CompleteTokenIds>(batch_size, batch_size, max_row_len, cache_config_.seq_size_per_block);
    complete_token_ids->initFromRows(full_token_rows, reuse_len);

    MallocInfo malloc_info;
    malloc_info.batch_kv_cache_resource = kv_resource;
    malloc_info.complete_token_ids      = complete_token_ids;
    malloc_info.request_id              = input->request_id;
    malloc_info.reuse_cache             = false;
    malloc_info.enable_device_cache     = false;
    auto malloc_result                  = cache_manager_->malloc(malloc_info);
    if (!malloc_result.success) {
        RTP_LLM_LOG_WARNING("mainse prefix kv cache malloc failed, fallback to normal embedding path");
        return processNormal({stream});
    }

    auto free_cache = [&]() {
        FreeInfo free_info;
        free_info.batch_kv_cache_resource = kv_resource;
        free_info.complete_token_ids      = complete_token_ids;
        free_info.request_id              = input->request_id;
    ...
    prefix_input->prefix_lengths    = torch::zeros({1}, torch::kInt32);
    prefix_input->kv_cache_resource = kv_resource;
    std::list<EmbeddingStreamPtr> prefix_streams{std::make_shared<EmbeddingStream>(prefix_input)};

    auto prefix_model_input_status = gatherModelInput(prefix_streams);
    if (!prefix_model_input_status.ok()) {
        free_cache();
        return prefix_model_input_status.status();
    }
    auto prefix_model_input = std::move(prefix_model_input_status.value());
    model_->releaseBuffers();
    ...
    for (int i = 0; i < batch_size; ++i) {
        ...
        suffix_tokens.insert(suffix_tokens.end(), full_token_rows[i].begin() + reuse_len, full_token_rows[i].end());
        suffix_types.insert(suffix_types.end(), full_type_rows[i].begin() + reuse_len, full_type_rows[i].end());
    ...
    suffix_input->prefix_lengths =
        torch::from_blob(suffix_prefix_lengths.data(), {(int64_t)suffix_prefix_lengths.size()}, torch::kInt32).clone();
    suffix_input->kv_cache_resource = kv_resource;
    std::list<EmbeddingStreamPtr> suffix_streams{std::make_shared<EmbeddingStream>(suffix_input)};

    auto suffix_model_input_status = gatherModelInput(suffix_streams);
    if (!suffix_model_input_status.ok()) {
        free_cache();
        return suffix_model_input_status.status();
    }
    auto suffix_model_input = std::move(suffix_model_input_status.value());
