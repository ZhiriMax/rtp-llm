...
__device__ __forceinline__ float GroupReduceMax(float val, const int tid) {
    ...
    static_assert((THREADS_PER_SUBWARP & (THREADS_PER_SUBWARP - 1)) == 0 && THREADS_PER_SUBWARP <= 16
                      && THREADS_PER_SUBWARP >= 1,
...
struct NaiveScheduler {
    ...
    __device__ __forceinline__ static void execute(const int      subwarps_per_block,
...
#pragma unroll
                for (uint32_t j = 0; j < INPUT_PRIMARY_INT4_SIZE; ++j) {
                    input_secondary_int4[j] = ld_global_nc(
                        reinterpret_cast<const int4*>(input + input_group_start_offset
                                                      + lane_id * INPUT_PRIMARY_VEC_SIZE + secondary_offset)
                        + j);
                }
            }
            ...
            if constexpr (IS_COLUMN_MAJOR and SCALE_UE8M0) {
                ...
                if ((remainder_num_groups != 0) and (hidden_dim_group_idx == hidden_dim_num_groups - 1)
                    and (lane_id < num_elems_per_pack - remainder_num_groups)) {
...
#pragma unroll
                for (uint32_t j = 0; j < INPUT_PRIMARY_VEC_SIZE; j += 2) {
                    float2 inputx2  = {static_cast<float>(input_primary_vec[j]),
...
#define LAUNCH_KERNEL_INNER(SCHEDULER, GROUP_SIZE, THREADS_PER_SUBWARP, T, DST_DTYPE, output_s_dtype, ...)             \
    do {                                                                                                               \
        int  subwarps_per_block;                                                                                       \
        dim3 grid, block;                                                                                              \
        SCHEDULER::compute_exec_config(THREADS_PER_SUBWARP,                                                            \
                                       num_local_experts,                                                              \
                                       hidden_dim_num_groups,                                                          \
                                       num_groups,                                                                     \
                                       subwarps_per_block,                                                             \
