...
load(
    "//3rdparty/cuda_config:common.bzl",
    "config_repo_label",
    "err_out",
    "execute",
    "get_bash_bin",
...
def _tpl(repository_ctx, tpl, substitutions = {}, out = None):
    ...
    repository_ctx.template(
        out,
        Label("//3rdparty/cuda_config/%s.tpl" % tpl),
        substitutions,
    )
...
def _file(repository_ctx, label):
    repository_ctx.template(
        label.replace(":", "/"),
        Label("//3rdparty/cuda_config/%s.tpl" % label),
        {},
    )
...
""",
    ...
    repository_ctx.file("cuda/cuda/include/nvml.h")
...
def _tpl_path(repository_ctx, filename):
    return repository_ctx.path(Label("//3rdparty/cuda_config/%s.tpl" % filename))
...
def _create_local_cuda_repository(repository_ctx):
    ...
    find_cuda_config_script = repository_ctx.path(Label("//3rdparty/cuda_config:find_cuda_config.py"))
    ...
    # copy files mentioned in 3rdparty/nccl/build_defs.bzl.tpl
...
remote_cuda_configure = repository_rule(
    ...
    attrs = {
        ...
        default = Label("//3rdparty/cuda_config:find_cuda_config.py")),
    },
)
